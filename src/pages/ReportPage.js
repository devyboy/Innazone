		...
		reportRef.get().then(doc => {
			if (!doc.exists) {
				...
				this.setState({ data: doc.data() }, () => {
					...
					this.map.setView(new OlView({ center: fromLonLat([this.state.data.longitude, this.state.data.latitude]), zoom: 16 }));
				});
			}
		});
	}
		...
		return (
			<div className="App" style={{ height: "100%", paddingBottom: "2em" }}>
				...
				<Modal show={this.state.deleteModal} onHide={() => this.setState({ deleteModal: false })}>
					...
					<Modal.Body>
						Are you sure you want to delete this report? This action is permanent. To proceed, enter the pasword
						you used to create it below and click "Delete".
