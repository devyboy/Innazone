...
const passPopover = (
  <Popover id="popover-basic">
    ...
    <Popover.Content>
      This password will be used to generate a <strong>tripcode</strong>. The
      tripcode will be used in conjunction with the provided name to uniquely
      identify your reports and prevent impersonation.
    </Popover.Content>
  </Popover>
);
...
class StashForm extends React.Component {
  ...
  addItem(event) {
    if (event.keyCode === 13 || event.type === "blur") {
      if (
        !this.state.items.includes(event.target.value) &&
        event.target.value !== ""
      ) {
        this.setState(state => ({
  ...
  deleteItem(delItem) {
    this.setState(state => ({
      items: state.items.filter(
        (
          item // filter out the SLO's that aren't the one you want to delete
        ) => item !== delItem
      )
    }));
  }
            ...
            <Form.Row>
              <Form.Group as={Col} controlId="formGridItems">
                ...
                <Form.Control
                  ...
                  style={
                    this.state.itemsError ? { border: "2px solid red" } : null
                  }
                />
              </Form.Group>
            </Form.Row>
